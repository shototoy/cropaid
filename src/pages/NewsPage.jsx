...
export default function NewsPage() {
    ...
    // Generate news from verified reports
    const generateNewsFromReports = (reports) => {
        if (!reports || reports.length === 0) return [];
        
        // Group reports by type and count
        const grouped = reports.reduce((acc, report) => {
            const type = report.type || report.report_type;
            if (!acc[type]) acc[type] = { count: 0, locations: new Set() };
            acc[type].count++;
            if (report.location || report.farm_barangay) {
                acc[type].locations.add(report.location || report.farm_barangay);
            }
            return acc;
        }, {});
        ...
        Object.entries(grouped).forEach(([type, data]) => {
            if (data.count > 0) {
                ...
                generatedNews.push({
                    ...
                    summary: `Recent ${type} incidents have been verified in ${locations || 'the municipality'}. Farmers in affected areas should take appropriate precautions.`,
                    content: `The Municipal Agriculture Office has verified ${data.count} ${type} report${data.count > 1 ? 's' : ''} from farmers in ${locations || 'various barangays'}. Affected farmers are advised to implement recommended countermeasures and report any additional incidents through the CropAid system.`,
    ...
    useEffect(() => {
        const fetchNews = async () => {
            ...
            try {
                // Fetch regular advisories
                const advisoriesPromise = fetch(`${API_URL}/advisories`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });

                // Also fetch recent verified reports for auto-generated news
                const reportsPromise = fetch(`${API_URL}/farmer/reports?status=verified&limit=20`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });

                const [advisoriesRes, reportsRes] = await Promise.all([advisoriesPromise, reportsPromise]);
                
                let allNews = [];
                
                // Add regular advisories
                if (advisoriesRes.ok) {
                    const data = await advisoriesRes.json();
                    allNews = data.advisories || data || [];
                }

                // Generate news from verified reports
                if (reportsRes.ok) {
                    const reportsData = await reportsRes.json();
                    const verifiedReports = (reportsData.reports || reportsData || [])
                        .filter(r => r.status === 'verified' || r.status === 'Verified');
                    const autoNews = generateNewsFromReports(verifiedReports);
                    allNews = [...autoNews, ...allNews];
                }

                // Sort by date descending
                allNews.sort((a, b) => new Date(b.date) - new Date(a.date));
                
                setNews(allNews.length > 0 ? allNews : MOCK_NEWS);
            } catch (err) {
